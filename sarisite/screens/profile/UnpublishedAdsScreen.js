// screens/profile/UnpublishedAdsScreen.js
...
export default function UnpublishedAdsScreen({ navigation }) {
  ...
  const fetchAds = async () => {
    try {
      setLoading(true);
      const fetchedAds = await get('/users/me/ads?active=false', true);
      setAds(fetchedAds);
    } catch (error) {
  ...
  const handleDelete = (adId) => {
    Alert.alert('İlanı Sil', 'Bu ilanı kalıcı olarak silmek istediğinizden emin misiniz?',
      [{ text: 'Vazgeç' }, { text: 'Sil', onPress: async () => {
            try {
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.emptyText}>Yayında olmayan ilanınız bulunmuyor.</Text>}
